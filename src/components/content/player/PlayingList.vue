<template>
	<div class="playing-list" id="playing-song-list">
		...
		<div class="play-list" v-if="activeItem == 0">
			...
			<div class="bd">
				<el-table size="mini" :show-header="false" style="width: 100%" :row-class-name="tableRowClassName"
					:data="playingList" @row-dblclick="playSongClick" empty-text="暂无歌曲">
		...
		<div class="history-list" v-if="activeItem == 1">
			...
			<div class="bd">
				<el-table size="mini" :show-header="false" style="width: 100%" :row-class-name="tableRowClassName"
					:data="historyPlay" @row-dblclick="playHistorySong" empty-text="暂无记录">
					<el-table-column width="50">
						<template v-slot="scope">
							<span class="iconfont" :class="isPlaying ? ' icon-voice' : 'icon-jingyin_laba'"
								v-if="scope.row.id == nowSongDetail.id"></span>
...
import TabBar from "@/components/content/songlist/base_tabBar.vue";
export default {
	name: "PlayingList",
	components: { TabBar },
	computed: {
		...mapState(["playingList", "historyPlay", "nowSongDetail", "isPlaying"]),
	},
	data() {
		return {
