<template>
	<div class="comment">
		<!-- 输入框 -->
		<div class="textinput">
			<el-input ref="commentContent" type="textarea" rows="3" maxlength="100" show-word-limit v-model.trim="content"
				:placeholder="placeholder" @blur="textareaBlur"> </el-input>
			<el-button round type="primary" size="small" icon="el-icon-position" class="el-btn" @click="submitComment">发送评论
			</el-button>
		</div>
		<!-- 评论列表 -->
		<div class="comment-list" ref="commentlist">
			...
			<div class="sort-category">
				<span v-for="(item, index) in sortCategory" :key="index" @click="changeSort(item.sortType, index)"
					:class="{ active: currentIndex === index }">{{ item.title }}</span>
			</div>
			<div class="new-comment" v-if="commentsList.length !== 0">
				<CommentItem v-for="(item, index) in commentsList" :key="index" :item="item" @toUserDetail="toUserDetail"
					@getReplyComment="getReplyComment" :commentType="commentType" :commentresId="commentresId" />
				<Pagination :total="commentCount" :page-size="60" :current-page="currentPage" :scroll-top="false"
