    ...
    <div class="logined" v-if="$store.state.isLogin">
      ...
      <div class="top-userinfo">
        ...
        <div class="right-info">
          ...
          <div class="row1">
            ...
            <!-- 关注按钮 -->
            <el-button size="small" v-if="uid != $store.state.userInfo.userId" class="subuser"
              :icon="isSub ? 'el-icon-check' : 'el-icon-plus'" :type="isSub ? 'primary' : 'default'" @click="subUserBy">{{
          ...
          <div class="my-data">
            <div class="data-info">
              <div class="data-num">
                ...
                <div class="num-item" style="border-right: solid 1px #dddddd; border-left: solid 1px #dddddd"
                  @click="toFollows">
      ...
      <div class="mycreat" v-if="createdSongList.length !== 0">
        <el-divider content-position="left">
          <h2>
            <span>{{ currentUserInfo.nickname }}</span>创建的歌单({{ createdSongList.length }})
          </h2>
        </el-divider>
        <SongList :singlist="createdSongList" />
      </div>
      <div class="mycollect" v-if="collectSongList.length !== 0">
        <el-divider content-position="left">
          <h2>
            <span>{{ currentUserInfo.nickname }}</span>收藏的歌单({{ collectSongList.length }})
          </h2>
        </el-divider>
        <SongList :singlist="collectSongList" />
      </div>
    </div>
...
import SongList from "../base/base_singsheet.vue";
export default {
  inject: ["reload"],
  name: "UserDetail",
  components: { SongList },
